$(function () {
    ...
    hub.on("NewMessage", function (message) {
        ...
        if (message.senderId.toString() == userId)
            ...
            messagesBox.append(CreateAdminMessageBlock(message));
        else
    ...
    hub.on("LoadMessages", function (messages) {
        ...
        for (message of messages) {
            if (message.senderId.toString() == userId)
                ...
                messagesBox.prepend(CreateAdminMessageBlock(message));
            else
    ...
    function CreateAdminMessageBlock(message) {
        var messageHtml = `<div class="message admin"><div>Администратор</div><div>${message.message}</div></div>`
        return $(messageHtml);
